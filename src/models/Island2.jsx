...
const Island2 = ({ isRotating, setIsRotating, setCurrentStage, currentFocusPoint, onIslandClick, ...props }) => {
  ...
  // Track if we're dragging to prevent click on drag
  const isDragging = useRef(false);
  const dragThreshold = useRef(5); // pixels
  ...
  // Handle click events properly - SEPARATE from rotation events
  const handleMeshClick = (e) => {
    console.log("🎯 Mesh click detected!");
    console.log("isDragging.current:", isDragging.current);
    console.log("hasMovedEnough.current:", hasMovedEnough.current);
    
    // R3F events have stopPropagation, but might not have preventDefault
    if (e.stopPropagation) e.stopPropagation();
    
    // Only trigger click if we haven't dragged significantly
    if (!hasMovedEnough.current && !isDragging.current) {
      console.log("✅ Valid click - calling onIslandClick");
      if (onIslandClick) {
        onIslandClick();
      }
    } else {
  ...
  // R3F event handlers - these get R3F's ThreeEvent objects
  const handleR3FPointerDown = (e) => {
    console.log("👇 R3F Pointer down");
    safePreventDefault(e);
    
    // Get client coordinates from R3F event
  ...
  const handleR3FPointerUp = (e) => {
    console.log("👆 R3F Pointer up");
    safePreventDefault(e);
    setIsRotating(false);
    
    // Don't reset immediately - let click events fire first
    setTimeout(() => {
      isDragging.current = false;
      hasMovedEnough.current = false;
    }, 100);
  ...
  const handleR3FPointerMove = (e) => {
    ...
    if (isRotating) {
      ...
      if (dragDistance > dragThreshold.current) {
  ...
  const handleR3FWheel = (e) => {
    // R3F wheel events might be different
    const deltaX = e.deltaX || (e.nativeEvent && e.nativeEvent.deltaX) || 0;
    const deltaY = e.deltaY || (e.nativeEvent && e.nativeEvent.deltaY) || 0;
  ...
  // DOM event handlers for keyboard
  ...
  return (
    <a.group 
      ref={islandRef} 
      {...props}
      onPointerDown={handleR3FPointerDown}
      onPointerUp={handleR3FPointerUp}
      onPointerMove={handleR3FPointerMove}
      onWheel={handleR3FWheel}
    >
      {/* Main clickable island group - tree with bark */}
      <a.group 
        position={[-36.773, 6.538, 16.547]}
        rotation={[Math.PI, -1.083, Math.PI]}
      >
        {/* This is your main tree trunk - make it clickable */}
        <mesh
          ...
          onPointerOver={(e) => {
            ...
            console.log("🖱️ Hovering over tree trunk");
          }}
          onPointerOut={(e) => {
            safePreventDefault(e);
            document.body.style.cursor = 'auto';
            console.log("🖱️ Left tree trunk");
          }}
        />
        
        {/* Leaves meshes - also make them clickable for larger click area */}
        <mesh
      ...
      <a.group position={[-45.385, 8.643, -9.317]} rotation={[0.082, -1.052, 0.184]}>
        <mesh
      ...
      <a.group position={[0.042, 8.557, 42.483]} rotation={[0.016, -0.851, -0.035]}>
        <mesh
      ...
      <a.group position={[-30.433, 8.865, 30.478]} rotation={[-1.41, -0.182, -2.308]}>
        <mesh
      ...
      <a.group position={[-46.367, 8.827, -31.152]} rotation={[0.066, 1.329, -0.124]}>
        <mesh
        ...
        />
        <a.group position={[0, -2.99, 0]} rotation={[0, -1.271, 0]}>
          <mesh
        ...
        <a.group position={[0, -1.933, 0]} rotation={[0, -1.271, 0]}>
          <mesh
      ...
      <a.group position={[44.725, 7.058, -24.956]} rotation={[-2.462, 0.262, -2.885]}>
        <mesh
      ...
      <a.group position={[2.042, 61.728, -0.692]}>
        <mesh
        ...
        <mesh
          ...
          material={materials['rocks.003']}
        />
        <a.group position={[-0.758, 5.069, -3.95]} rotation={[3.14, 0.016, -3.113]}>
          <mesh
        ...
        <a.group position={[-2.21, 5.019, 3.348]} rotation={[0, -0.754, 0]}>
          <mesh
        ...
        <a.group position={[-3.947, 5.077, -0.736]} rotation={[Math.PI, -1.166, Math.PI]}>
          <mesh
        ...
        <a.group position={[-3.977, 0.769, 0.025]} rotation={[Math.PI, -1.206, Math.PI]}>
          <mesh
        ...
        <a.group position={[-5.53, -3.537, 0.348]} rotation={[0, 0.486, 0]}>
          <mesh
        ...
        <a.group position={[-5.715, -3.905, 0.058]} rotation={[0, 0.486, 0]}>
          <mesh
        ...
        <a.group position={[-5.963, -4.267, -0.377]} rotation={[0, 0.486, 0]}>
          <mesh
      ...
      <a.group position={[-16.418, 6.649, -57.221]} rotation={[-0.159, -0.14, 0.147]}>
        <mesh
      ...
      <a.group position={[-0.427, 5.5, -56.782]} rotation={[-0.543, 0.305, 0.056]}>
        <mesh
      ...
      <a.group position={[25.178, 5.377, -46.814]} rotation={[-0.694, 0.692, 0.365]}>
        <mesh
      ...
      <a.group position={[-3.955, 56.529, -6.364]} rotation={[-0.386, 0.5, 0.482]}>
        <mesh
      ...
      <a.group position={[-7.668, 49.339, -9.981]} rotation={[-0.667, 0.286, 0.771]}>
        <mesh
      ...
      />
           ...
           {/* Debug sphere to test clicking - remove this later */}
      <mesh position={[0, 20, 0]} onClick={handleMeshClick}>
